...
int CAddrInfo::GetNewBucket(const uint256& nKey, const CNetAddr& src) const {
	...
	uint64_t hash1 = (CHashWriter(SER_GETHASH, 0) << nKey << GetGroup() << vchSourceGroupKey)
	                     .GetHash()
	                     .GetCheapHash();
	uint64_t hash2 = (CHashWriter(SER_GETHASH, 0) << nKey << vchSourceGroupKey
...
int CAddrInfo::GetBucketPosition(const uint256& nKey, bool fNew, int nBucket) const {
	uint64_t hash1 =
	    (CHashWriter(SER_GETHASH, 0) << nKey << (fNew ? 'N' : 'K') << nBucket << GetKey())
	        .GetHash()
	        .GetCheapHash();
...
bool CAddrInfo::IsTerrible(int64_t nNow) const {
	...
	if (nTime == 0 ||
	    nNow - nTime > ADDRMAN_HORIZON_DAYS * 24 * 60 * 60)  // not seen in recent history
		return true;
...
double CAddrInfo::GetChance(int64_t nNow) const {
	...
	// deprioritize 66% after each failed attempt, but at most 1/28th to avoid the search taking
	// forever or overly penalizing outages.
...
void CAddrMan::MakeTried(CAddrInfo& info, int nId) {
	...
	// first make space to add it (the existing tried entry there is moved to new, deleting whatever
	// is there).
...
bool CAddrMan::Add_(const CAddress& addr, const CNetAddr& source, int64_t nTimePenalty) {
	...
	if (pinfo) {
		...
		if (addr.nTime &&
		    (!pinfo->nTime || pinfo->nTime < addr.nTime - nUpdateInterval - nTimePenalty))
