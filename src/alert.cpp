...
std::string CUnsignedAlert::ToString() const {
	...
	return strprintf(
	    ...
	    "    strStatusBar = \"%s\"\n"
	    ")\n",
	    nVersion, nRelayUntil, nExpiration, nID, nCancel, strSetCancel, nMinVer, nMaxVer,
	    strSetSubVer, nPriority, strComment, strStatusBar);
}
...
bool CAlert::ProcessAlert(bool fThread) {
	...
	if (nID == maxInt) {
		if (!(nExpiration == maxInt && nCancel == (maxInt - 1) && nMinVer == 0 &&
		      nMaxVer == maxInt && setSubVer.empty() && nPriority == maxInt &&
