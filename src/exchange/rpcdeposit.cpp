...
void printpegtxout(const CPegData& pegdata, Object& result, string prefix);
...
Value registerdeposit(const Array& params, bool fHelp) {
	...
	if (nDepth < nRecommendedConfirmations) {
		...
		result.push_back(
		    Pair("status", strprintf("Need at least %d confirmations", nRecommendedConfirmations)));
		return result;
	}
	...
	if (nRegisterHeight > nBestHeight) {
		...
		result.push_back(Pair(
		    "status", strprintf("Need to wait for registration at %d block", nRegisterHeight)));
		return result;
	}
	...
	pdTxout.fractions.sReturnAddr =
	    HexStr(tx.vout[nout].scriptPubKey.begin(), tx.vout[nout].scriptPubKey.end());
