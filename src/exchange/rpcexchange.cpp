...
void printpeglevel(const CPegLevel& peglevel, Object& result);
...
Value getpeglevel(const Array& params, bool fHelp) {
	...
	if (nCycleNow <= nCyclePrev) {
		std::stringstream ss;
		ss << "Current cycle " << nCycleNow << " should be greater than previous " << nCyclePrev;
		string err = ss.str();
	...
	bool ok = pegops::getpeglevel(nCycleNow, nCyclePrev, nBuffer, nSupplyNow, nSupplyNext,
	                              nSupplyNextNext, pdExchange, pdPegShift,

	                              peglevel, pdPegPool, err);
	if (!ok) {
		throw JSONRPCError(RPC_MISC_ERROR, err);
	}
...
Value makepeglevel(const Array& params, bool fHelp) {
	...
	bool ok = pegops::getpeglevel(cycle_now, cycle_prev, buffer, supply_now, supply_next,
	                              supply_next_next, pdExchange, pdPegShift,
...
Value updatepegbalances(const Array& params, bool fHelp) {
	...
	bool ok = pegops::updatepegbalances(pdBalance, pdPegPool, peglevelNew,
...
Value movecoins(const Array& params, bool fHelp) {
	...
	bool ok = pegops::movecoins(inp_move_amount, pdSrc, pdDst, peglevel, true,
...
Value moveliquid(const Array& params, bool fHelp) {
	...
	bool ok = pegops::moveliquid(inp_move_liquid, pdSrc, pdDst, peglevel,
...
Value movereserve(const Array& params, bool fHelp) {
	...
	bool ok = pegops::movereserve(inp_move_reserve, pdSrc, pdDst, peglevel,
