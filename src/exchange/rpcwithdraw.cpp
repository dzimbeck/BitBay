...
void printpegshift(const CFractions& frPegShift, const CPegLevel& peglevel, Object& result);
...
static void consumereservepegshift(CFractions&      frBalance,
	...
	if (frPegShift.Positive(nullptr).Total() != -frPegShift.Negative(nullptr).Total()) {
		throw JSONRPCError(RPC_MISC_ERROR, strprintf("Mismatch pegshift parts (%d - %d)",
...
static void cleanupConsumed(const set<uint320>& setConsumedInputs,
	...
	std::set_intersection(setConsumedInputs.begin(), setConsumedInputs.end(), setAllOutputs.begin(),
	                      setAllOutputs.end(),
...
static void cleanupProvided(const set<uint320>&       setWalletOutputs,
	...
	for (const pair<uint320, CCoinToUse>& item : mapProvidedOutputs) {
		if (setWalletOutputs.count(item.first))
			continue;
...
static void getAvailableCoins(const set<uint320>&       setConsumedInputs,
	...
	for (const COutput& coin : vecCoins) {
		...
		if (setConsumedInputs.count(fkey))
...
static void computeTxPegForNextCycle(const CTransaction&       rawTx,
	...
	if (!peg_ok) {
		throw JSONRPCError(
		    RPC_MISC_ERROR,
		    strprintf("Fail on calculations of tx fractions (cause=%s)", sPegFailCause.c_str()));
	}
...
static void prepareConsumedProvided(map<uint320, CCoinToUse>& mapProvidedOutputs,
	...
	for (size_t i = 0; i < rawTx.vin.size(); i++) {
		const COutPoint& prevout = rawTx.vin[i].prevout;
		auto             fkey    = uint320(prevout.hash, prevout.n);
		if (mapProvidedOutputs.count(fkey))
			mapProvidedOutputs.erase(fkey);
		if (!sConsumedInputs.empty())
			sConsumedInputs += ",";
		sConsumedInputs += fkey.GetHex();
	}
	...
	sProvidedOutputs +=
	    "accountmaintenance:" + std::to_string(nMaintenance) + ":" + pdSave.ToString();
}
...
Value prepareliquidwithdraw(const Array& params, bool fHelp) {
	...
	if (nAmountWithFee < 0) {
		throw JSONRPCError(RPC_MISC_ERROR,
		                   strprintf("Requested to withdraw negative %d", nAmountWithFee));
	}
	if (nAmountWithFee < 1000000 + 2) {
		throw JSONRPCError(RPC_MISC_ERROR,
		                   strprintf("Requested to withdraw less than 1M fee %d", nAmountWithFee));
	}
	...
	// network peglevel
	CPegLevel peglevel_net(nCycleNow, nCycleNow - 1, 0 /*buffer=0, network level*/, nSupplyNow,
	                       nSupplyNext, nSupplyNextNext);
	...
	if (fPartial) {
		...
		if (nPartialLiquid < 0) {
			throw JSONRPCError(RPC_MISC_ERROR,
			                   strprintf("Mismatch on nPartialLiquid %d", nPartialLiquid));
		}
	...
	if (frBalanceLiquid.Total() < nAmountWithFee) {
		throw JSONRPCError(RPC_MISC_ERROR, strprintf("Not enough liquid(1) %d  on 'src' to move %d",
		                                             frBalanceLiquid.Total(), nAmountWithFee));
	}
	...
	set<uint320>             setConsumedInputs;
	map<uint320, CCoinToUse> mapProvidedOutputs;
	parseConsumedAndProvided(sConsumedInputs, sProvidedOutputs, nCycleNow, nMaintenance,
	                         frMaintenance, setAllOutputs, setConsumedInputs, mapProvidedOutputs);
	...
	// get available coins
	getAvailableCoins(setConsumedInputs, nCycleNow, setAllOutputs, setWalletOutputs, mapAllOutputs);
	...
	map<int, CFractions> mapTxOutputFractions;
	computeTxPegForNextCycle(rawTx, peglevel_net, txdb, pegdb, mapAllOutputs, mapTxOutputFractions,
	                         feesFractionsNet);
	...
	for (const CCoinToUse& coin : vCoins) {
		if (!SignSignature(*pwalletMain, coin.scriptPubKey, rawTx, nIn++)) {
			throw JSONRPCError(RPC_MISC_ERROR, strprintf("Fail on signing input (%d)", nIn - 1));
		}
	}
	// for liquid just first output
	...
	prepareConsumedProvided(mapProvidedOutputs, rawTx, sAddress, nCycleNow, nMaintenance,
	                        frMaintenance, peglevel_exchange, sConsumedInputs, sProvidedOutputs);
	...
	consumeliquidpegshift(pdBalance.fractions, pdExchange.fractions, pdPegShift.fractions,
	                      peglevel_exchange);
...
Value preparereservewithdraw(const Array& params, bool fHelp) {
	...
	if (fPartial) {
		...
		if (nPartialReserve < 0) {
			throw JSONRPCError(RPC_MISC_ERROR,
			                   strprintf("Mismatch on nPartialReserve %d", nPartialReserve));
		}
	...
	{
		CPegDB pegdbrw;
		for (size_t i = rawTx.vin.size() + 1; i < rawTx.vout.size();
		     i++) {  // skip F notations and withdraw
			// save these outputs in pegdb, so they can be used in next withdraws
			auto fkey = uint320(rawTx.GetHash(), i);
			pegdbrw.WriteFractions(fkey, mapTxOutputFractions[i]);
		}
	...
	consumereservepegshift(pdBalance.fractions, pdExchange.fractions, pdPegShift.fractions,
	                       peglevel_exchange);
...
Value accountmaintenance(const Array& params, bool fHelp) {
	...
	// network peglevel
	CPegLevel peglevel_net(nCycleNow, nCycleNow - 1, 0, nSupplyNow, nSupplyNext, nSupplyNextNext);
	...
	//    {
	//        CPegDB pegdbrw;
	//        for (size_t i=rawTx.vin.size()+1; i< rawTx.vout.size(); i++) { // skip notations and
	//        withdraw
