...
public:
	...
	bool Verify(const uint256& hash, const std::vector<unsigned char>& vchSig) {
		...
		if (ECDSA_verify(0, (unsigned char*)&hash, sizeof(hash), &vchSig[0], vchSig.size(), pkey) !=
		    1)
			return false;
		return true;
	}
	...
	bool SignCompact(const uint256& hash, unsigned char* p64, int& rec) {
		...
		if (nBitsR <= 256 && nBitsS <= 256) {
			...
			for (int i = 0; i < 4; i++) {
				CECKey keyRec;
				if (ECDSA_SIG_recover_key_GFp(keyRec.pkey, sig, (unsigned char*)&hash, sizeof(hash),
				                              i, 1) == 1) {
					CPubKey pubkeyRec;
...
bool CKey::Check(const unsigned char* vch) {
	...
	static const unsigned char vchMax[32] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
