...
void CNode::PushVersion() {
	...
	CAddress addrYou =
	    (addr.IsRoutable() && !IsProxy(addr) ? addr : CAddress(CService("0.0.0.0", 0)));
	...
	PushMessage("version", PROTOCOL_VERSION, nLocalServices, nTime, addrYou, addrMe,
	            nLocalHostNonce,
	            FormatSubVersion(CLIENT_NAME, CLIENT_VERSION, std::vector<string>()), nBestHeight);
}
...
bool CNode::Misbehaving(int howmuch) {
	...
	if (nMisbehavior >= GetArg("-banscore", 100)) {
		...
		LogPrintf("Misbehaving: %s (%d -> %d)\n", addr.ToString(), nMisbehavior - howmuch,
		          nMisbehavior);
	return false;
}
...
void CNode::copyStats(CNodeStats& stats) {
	...
	// Raw ping time is in microseconds, but show it to user as whole seconds (Bitcoin users should
	// be well used to small numbers with many decimal places by now :)
	...
	while (true) {
		...
		if (!changed) {
			...
			for (size_t i = 0; i < vNodes.size(); i++) {
				...
				if (pNode->addrName != vPrevStats[i].addrName) {
					changed = true;
					break;
				}
				if (pNode->nVersion != vPrevStats[i].nVersion) {
					changed = true;
					break;
				}
				if (pNode->strSubVer != vPrevStats[i].strSubVer) {
					changed = true;
					break;
				}
				if (pNode->nStartingHeight != vPrevStats[i].nStartingHeight) {
					changed = true;
					break;
				}
			}
...
void ThreadMapPort() {
	...
	if (r == 1) {
		...
		string strDesc = "BitBay " + FormatFullVersion();
...
void MapPort(bool fUseUPnP) {
	...
	if (fUseUPnP) {
		...
		upnp_thread =
		    new boost::thread(boost::bind(&TraceThread<void (*)()>, "upnp", &ThreadMapPort));
	} else if (upnp_thread) {
		upnp_thread->interrupt();
		upnp_thread->join();
		delete upnp_thread;
...
void ThreadOpenConnections() {
	...
	while (true) {
		...
		while (true) {
			...
			// addrman, stop this loop, and let the outer loop run again (which sleeps, adds seed
			// nodes, recalculates already-connected network ranges, ...) before trying new addrman
			// addresses.
			nTries++;
			if (nTries > 100)
				break;
...
bool BindListenPort(const CService& addrBind, string& strError) {
	...
	if (!addrBind.GetSockAddr((struct sockaddr*)&sockaddr, &len)) {
		strError =
		    strprintf("Error: bind address family for %s not supported", addrBind.ToString());
		LogPrintf("%s\n", strError);
		return false;
	}
	...
	if (addrBind.IsIPv6()) {
		...
		setsockopt(hListenSocket, IPPROTO_IPV6, nParameterId, (const char*)&nProtLevel,
		           sizeof(int));
#endif
	}
	...
	if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
		...
		if (nErr == WSAEADDRINUSE)
			strError = strprintf(
			    _("Unable to bind to %s on this computer. BitBay is probably already running."),
			    addrBind.ToString());
		else
			strError =
			    strprintf(_("Unable to bind to %s on this computer (bind returned error %d, %s)"),
	...
	if (listen(hListenSocket, SOMAXCONN) == SOCKET_ERROR) {
		strError =
		    strprintf("Error: Listening for incoming connections failed (listen returned error %d)",
		              WSAGetLastError());
		LogPrintf("%s\n", strError);
		return false;
	}
...
void StartNode(boost::thread_group& threadGroup) {
	...
	if (pnodeLocalHost == NULL)
		pnodeLocalHost =
		    new CNode(INVALID_SOCKET, CAddress(CService("127.0.0.1", 0), nLocalServices));
