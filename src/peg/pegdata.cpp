...
int64_t RatioPart(int64_t nValue, int64_t nPartValue, int64_t nTotalValue) {
...
CPegData::CPegData(std::string pegdata64) {
	...
	CDataStream finp(pegdata.data(), pegdata.data() + pegdata.size(), SER_NETWORK, CLIENT_VERSION);
	bool        ok = Unpack(finp);
	if (!ok) {
		// try prev versions
		CDataStream finp(pegdata.data(), pegdata.data() + pegdata.size(), SER_DISK, CLIENT_VERSION);
...
bool CPegData::IsValid() const {
	...
	// match total
	if ((nReserve + nLiquid) != fractions.Total())
		return false;
	...
	if (fPartial) {
		...
		if (nLiquid < nLiquidWithoutPartial)
			return false;
		if (nReserve < nReserveWithoutPartial)
			return false;
	} else {
		int64_t nLiquidCalc  = fractions.High(nSupplyEffective);
		int64_t nReserveCalc = fractions.Low(nSupplyEffective);
		if (nLiquid != nLiquidCalc)
			return false;
		if (nReserve != nReserveCalc)
			return false;
	}
