...
public:
	...
	enum MarkAction { MARK_SET = 0, MARK_TRANSFER = 1, MARK_COLD_TO_FROZEN = 2 };
	enum { SER_MASK = 0xffff, SER_VALUE = (1 << 16), SER_ZDELTA = (1 << 17), SER_RAW = (1 << 18) };
	...
	CFractions&       operator-=(const CFractions& b);
	friend CFractions operator+(CFractions a, const CFractions& b) {
		a += b;
		return a;
	}
	friend CFractions operator-(CFractions a, const CFractions& b) {
	...
	int64_t MoveRatioPartTo(int64_t nPartValue, CFractions& b);
