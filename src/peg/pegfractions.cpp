...
CFractions::CFractions() : nFlags(VALUE), f(new int64_t[PEG_SIZE]) {
	f[0] = 0;  // fast init first item
}
CFractions::CFractions(int64_t value, uint32_t flags) : nFlags(flags), f(new int64_t[PEG_SIZE]) {
	if (flags & VALUE)
...
bool CFractions::Pack(CDataStream& out, unsigned long* report_len, bool compress) const {
	if (nFlags & VALUE) {
		...
		if (res == Z_OK) {
			...
			if (report_len)
				*report_len = PEG_SIZE * sizeof(int64_t);
			out << nVersion;
...
int64_t CFractions::Low(const CPegLevel& peglevel) const {
	...
	if (peglevel.nShiftLastPart > 0 && peglevel.nShiftLastTotal > 0) {
		// partial value to use
		int64_t v     = f[to];
		int64_t vpart = ::RatioPart(v, peglevel.nShiftLastPart, peglevel.nShiftLastTotal);
		if (vpart < v)
			vpart++;  // better rounding
		nValue += vpart;
	}
...
CFractions CFractions::LowPart(const CPegLevel& peglevel, int64_t* total) const {
	...
	int to = peglevel.nSupply + peglevel.nShift;
	if (to >= 0 && to < PEG_SIZE && peglevel.nShiftLastPart > 0 && peglevel.nShiftLastTotal > 0) {
		// partial value to use
		int64_t v     = f[to];
		int64_t vpart = ::RatioPart(v, peglevel.nShiftLastPart, peglevel.nShiftLastTotal);
		if (vpart < v)
...
CFractions CFractions::HighPart(const CPegLevel& peglevel, int64_t* total) const {
	...
	int from = peglevel.nSupply + peglevel.nShift;
	if (from >= 0 && from < PEG_SIZE && peglevel.nShiftLastPart > 0 &&
...
CFractions CFractions::MidPart(const CPegLevel& peglevel_low,
	...
	if (peglevel_low.nShiftLastPart > 0 && peglevel_low.nShiftLastTotal > 0) {
		int64_t vpart =
		    ::RatioPart(vone, peglevel_low.nShiftLastPart, peglevel_low.nShiftLastTotal);
		v -= vpart;
	}

	if (peglevel_high.nShiftLastPart > 0 && peglevel_high.nShiftLastTotal > 0) {
		int64_t vpart =
		    ::RatioPart(vone, peglevel_high.nShiftLastPart, peglevel_high.nShiftLastTotal);
		if (vpart < vone)
			vpart++;
		v -= (vone - vpart);
	}
 ...
 *  #NOTE8
 */
int64_t CFractions::MoveRatioPartTo(int64_t nValueToMove, CFractions& b) {
	int64_t nTotalValue = Total();
