...
bool getpeglevel(int             nCycleNow,
                 int             nCyclePrev,
                 int             nBuffer,
                 int             nPegNow,
                 int             nPegNext,
	...
	pdPegPool.peglevel = CPegLevel(nCycleNow, nCyclePrev, nBuffer, nPegNow, nPegNext, nPegNextNext,
	                               pdExchange.fractions, pdPegShift.fractions);
...
bool updatepegbalances(CPegData&        pdBalance,
                       CPegData&        pdPegPool,
	...
	if (nValue != 0 &&                    /*allow cross-cycle update as zero to take from pool*/
		...
		std::stringstream ss;
		ss << "Mismatch for peglevel_new.nCyclePrev " << peglevelNew.nCyclePrev
		   << " vs peglevel_old.nCycle " << pdBalance.peglevel.nCycle;
		sErr = ss.str();
		return false;
	}
	...
	if (nSupplyEffective < 0 && nSupplyEffective >= PEG_SIZE) {
		std::stringstream ss;
	...
	if (fPartial) {
		...
		int64_t nTakeLiquid = RatioPart(nLastLiquid, nLiquid, nLiquidPool);
	...
	if (nLiquidTodo > nLiquidPool) {  // exchange liquidity mismatch
		std::stringstream ss;
		ss << "Not enough liquid " << nLiquidPool << " on 'pool' to balance " << nLiquidTodo;
		sErr = ss.str();
		return false;
	}
	...
	if (nLiquidTodo > 0) {
		std::stringstream ss;
		ss << "Liquid not enough after MoveRatioPartTo " << nLiquidTodo;
		sErr = ss.str();
		return false;
	}
	...
	if (nValue != pdBalance.fractions.Total()) {
		std::stringstream ss;
		ss << "Balance mimatch after update " << pdBalance.fractions.Total() << " vs " << nValue;
		sErr = ss.str();
		return false;
	}
	...
	if ((pdBalance.nReserve + pdBalance.nLiquid) != pdBalance.fractions.Total()) {
		std::stringstream ss;
		ss << "Balance mimatch liquid+reserve after update " << pdBalance.nLiquid << "+"
		   << pdBalance.nReserve << " vs " << pdBalance.fractions.Total();
		sErr = ss.str();
		return false;
	}
	...
	if (fPartial) {
		...
		if (pdBalance.nLiquid < nLiquidWithoutPartial) {
			std::stringstream ss;
			ss << "Balance liquid less than without partial after update " << pdBalance.nLiquid
			   << " vs " << nLiquidWithoutPartial;
			sErr = ss.str();
			return false;
		}
		if (pdBalance.nReserve < nReserveWithoutPartial) {
			std::stringstream ss;
			ss << "Balance reserve less than without partial after update " << pdBalance.nReserve
			   << " vs " << nReserveWithoutPartial;
			sErr = ss.str();
			return false;
		}
	} else {
		...
		if (pdBalance.nLiquid != nLiquidCalc) {
			std::stringstream ss;
			ss << "Balance liquid mismatch calculated after update " << pdBalance.nLiquid << " vs "
			   << nLiquidCalc;
			sErr = ss.str();
			return false;
		}
		if (pdBalance.nReserve != nReserveCalc) {
			std::stringstream ss;
			ss << "Balance reserve mismatch calculated after update " << pdBalance.nReserve
			   << " vs " << nReserveCalc;
			sErr = ss.str();
			return false;
		}
...
bool movecoins(int64_t          nMoveAmount,
               CPegData&        pdSrc,
               CPegData&        pdDst,
	...
	if (!fCrossCycles && peglevel != pdSrc.peglevel) {
		std::stringstream ss;
		ss << "Outdated 'src' of cycle of " << pdSrc.peglevel.nCycle << ", current "
		   << peglevel.nCycle;
		sErr = ss.str();
		return false;
	}
	...
	if (nSrcValue < nMoveAmount) {
		std::stringstream ss;
		ss << "Not enough amount " << nSrcValue << " on 'src' to move " << nMoveAmount;
		sErr = ss.str();
		return false;
	}
	...
	if (!fCrossCycles && peglevel != pdDst.peglevel) {
		std::stringstream ss;
		ss << "Outdated 'dst' of cycle of " << pdDst.peglevel.nCycle << ", current "
		   << peglevel.nCycle;
		sErr = ss.str();
		return false;
	}
	...
	if (nIn != nOut) {
		std::stringstream ss;
		ss << "Mismatch in and out values " << nIn << " vs " << nOut;
		sErr = ss.str();
		return false;
	}
	...
	if (pdSrc.fractions.Total() != (pdSrc.nLiquid + pdSrc.nReserve)) {
		std::stringstream ss;
		ss << "Mismatch total " << pdSrc.fractions.Total() << " vs liquid/reserve 'src' values "
		   << " L:" << pdSrc.nLiquid << " R:" << pdSrc.nReserve;
		sErr = ss.str();
		return false;
	}
	...
	if (pdDst.fractions.Total() != (pdDst.nLiquid + pdDst.nReserve)) {
		std::stringstream ss;
		ss << "Mismatch total " << pdDst.fractions.Total() << " vs liquid/reserve 'src' values "
		   << " L:" << pdDst.nLiquid << " R:" << pdDst.nReserve;
		sErr = ss.str();
		return false;
	}
...
bool moveliquid(int64_t          nMoveAmount,
                CPegData&        pdSrc,
                CPegData&        pdDst,
                const CPegLevel& peglevel,
	...
	if (pdSrc.nLiquid < nMoveAmount) {
		std::stringstream ss;
		ss << "Not enough liquid " << pdSrc.nLiquid << " on 'src' to move " << nMoveAmount;
		sErr = ss.str();
		return false;
	}
	...
	if (pdSrc.nId != 0 && pdDst.nId != 0 && pdSrc.nId == pdDst.nId) {
		std::stringstream ss;
		ss << "Detected move between same user src id:" << pdSrc.nId << " vs dst id:" << pdDst.nId;
		sErr = ss.str();
		return false;
	}
	...
	if (frLiquid.Total() < nMoveAmount) {
		std::stringstream ss;
		ss << "Not enough liquid(1) " << frLiquid.Total() << " on 'src' to move " << nMoveAmount;
		sErr = ss.str();
		return false;
	}
...
bool movereserve(int64_t          nMoveAmount,
                 CPegData&        pdSrc,
                 CPegData&        pdDst,
                 const CPegLevel& peglevel,
	...
	if (pdSrc.nReserve < nMoveAmount) {
		std::stringstream ss;
		ss << "Not enough reserve " << pdSrc.nReserve << " on 'src' to move " << nMoveAmount;
		sErr = ss.str();
		return false;
	}
...
public:
	...
	friend bool operator<(const CCoinToUse& a, const CCoinToUse& b) {
		if (a.txhash < b.txhash)
			return true;
		if (a.txhash == b.txhash && a.i < b.i)
			return true;
		if (a.txhash == b.txhash && a.i == b.i && a.nValue < b.nValue)
			return true;
		if (a.txhash == b.txhash && a.i == b.i && a.nValue == b.nValue &&
		    a.nAvailableValue < b.nAvailableValue)
			return true;
		if (a.txhash == b.txhash && a.i == b.i && a.nValue == b.nValue &&
		    a.nAvailableValue == b.nAvailableValue && a.scriptPubKey < b.scriptPubKey)
			return true;
		if (a.txhash == b.txhash && a.i == b.i && a.nValue == b.nValue &&
		    a.nAvailableValue == b.nAvailableValue && a.scriptPubKey == b.scriptPubKey &&
		    a.nCycle < b.nCycle)
	...
	IMPLEMENT_SERIALIZE(READWRITE(txhash); READWRITE(i); READWRITE(nValue); READWRITE(scriptPubKey);
	                    READWRITE(nCycle);)
};
...
static void saveMaintenance(string& data, const CFractions& fractions, const CPegLevel& peglevel) {
	CPegData pd;
...
bool prepareliquidwithdraw(const std::vector<std::tuple<std::string, CPegData, std::string>>& txins,
                           CPegData&        pdBalance,
                           CPegData&        pdExchange,
                           CPegData&        pdPegShift,
	...
	CPegLevel peglevel_net(peglevel_exchange.nCycle, peglevel_exchange.nCyclePrev,
	...
	if (nAmountWithFee > pdBalance.nLiquid) {
		std::stringstream ss;
		ss << "Not enough liquid " << pdBalance.nLiquid << " on 'balance' to withdraw "
		   << nAmountWithFee;
		sErr = ss.str();
		return false;
	}
	...
	if (frBalanceLiquid.Total() < nAmountWithFee) {
		std::stringstream ss;
		ss << "Not enough liquid(1) " << frBalanceLiquid.Total() << "  on 'balance' to withdraw "
		   << nAmountWithFee;
		sErr = ss.str();
		return false;
	}
	...
	if (nLeftAmount > 0) {
		std::stringstream ss;
		ss << "Not enough liquid in inputs to next cycle withdraw " << nAmountWithFee;
		sErr = ss.str();
		return false;
	}
	...
	for (const CTxDestination& address : vInputAddresses) {
		if (!mapChangeOutputs.count(address))
			continue;
	...
	for (size_t i = 1; i < rawTx.vout.size() - 1; i++) {
		...
		pdTxout.fractions.sReturnAddr =
		    HexStr(rawTx.vout[i].scriptPubKey.begin(), rawTx.vout[i].scriptPubKey.end());
	...
	saveMaintenance(pdPegShift.fractions.sReturnAddr, frMaintenance, peglevel_net);
...
bool preparereservewithdraw(
	...
	if (nAmountWithFee > pdBalance.nReserve) {
		std::stringstream ss;
		ss << "Not enough reserve " << pdBalance.nReserve << " on 'balance' to withdraw "
		   << nAmountWithFee;
		sErr = ss.str();
		return false;
	}
	...
	if (frBalanceReserve.Total() < nAmountWithFee) {
		std::stringstream ss;
		ss << "Not enough reserve(1) " << frBalanceReserve.Total() << "  on 'balance' to withdraw "
		   << nAmountWithFee;
		sErr = ss.str();
		return false;
	}
	...
	if (nLeftAmount > 0) {
		std::stringstream ss;
		ss << "Not enough reserve in inputs to next cycle withdraw " << nAmountWithFee;
		sErr = ss.str();
		return false;
	}
