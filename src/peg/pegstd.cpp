...
bool CalculateStandardFractions(const CTransaction& tx,
	...
	for (uint32_t i = 0; i < n_vin; i++) {
		...
		nValueIn += nValue;
		auto sAddress = prevtxout.scriptPubKey.ToAddress();
		...
		if (frInp.Total() != prevtxout.nValue) {
			std::stringstream ss;
			ss << "P-I-3: Input fraction " << prevout.hash.GetHex() << ":" << prevout.n << " total "
			   << frInp.Total() << " mismatches prevout value " << prevtxout.nValue;
			sFailCause = ss.str();
			return false;
		}

		if (frInp.nFlags & CFractions::NOTARY_F) {
			if (frInp.nLockTime > nBlockTime) {
				if (setTimeLockPass.count(i) == 0) {
		...
		if (i < n_vout) {
			string sNotary;
			bool   fNotary = tx.vout[i].scriptPubKey.ToNotary(sNotary);
			...
			if (frInp.nFlags & CFractions::NOTARY_C) {
				...
				uint32_t nOutIndex   = i;  // same index as input
				int64_t  nValueOut   = tx.vout[size_t(nOutIndex)].nValue;
				string   sOutAddress = tx.vout[size_t(nOutIndex)].scriptPubKey.ToAddress();
				...
				if (frInp.sReturnAddr == sOutAddress) {
					...
					if (!frozenTxOut.fractions.SetMark(CFractions::MARK_COLD_TO_FROZEN,
					                                   CFractions::NOTARY_F, nBlockTime)) {
			...
			if (fNotary && (fNotaryF || fNotaryV || fNotaryL || fNotaryC)) {
				...
				for (const string& sOutputArg : vOutputArgs) {
					char* pEnd         = nullptr;
					long  nFrozenIndex = strtol(sOutputArg.c_str(), &pEnd, 0);
					bool  fValidIndex  = !(pEnd == sOutputArg.c_str()) && nFrozenIndex >= 0 &&
					                   size_t(nFrozenIndex) < n_vout;
					if (!fValidIndex) {
					...
					bool fMarkSet        = false;
					if (fNotaryF)
						fMarkSet = frozenTxOut.fractions.SetMark(CFractions::MARK_SET,
						                                         CFractions::NOTARY_F, nBlockTime);
					if (fNotaryV)
						fMarkSet = frozenTxOut.fractions.SetMark(CFractions::MARK_SET,
						                                         CFractions::NOTARY_V, nBlockTime);
					if (fNotaryL)
						fMarkSet = frozenTxOut.fractions.SetMark(CFractions::MARK_SET,
						                                         CFractions::NOTARY_L, nBlockTime);
					if (fNotaryC)
						fMarkSet = frozenTxOut.fractions.SetMark(CFractions::MARK_SET,
						                                         CFractions::NOTARY_C, nBlockTime);
					if (!fMarkSet) {
				...
				if (vFrozenIndexes.size() == 1) {
					...
					if (!fSharedFreeze) {
						if (fNotaryF) {
							...
							if (!frozenTxOut.fractions.SetMark(CFractions::MARK_SET,
							                                   CFractions::NOTARY_F, nBlockTime)) {
							...
							if (frozenTxOut.fractions.Total() != 0) {
								sFailCause =
								    "P-I-N-12: Cold notary output has already assigned value";
								return false;
							}
	...
	for (uint32_t i = 0; i < n_vout; i++) {
		...
		bool   fNotaryZ = boost::starts_with(sNotary, "**Z**");
		...
		if (fFreezeAll && poolFrozen.count(i)) {
			if (poolFrozen[i].fractions.Total() > 0) {
				...
				if (poolFrozen[i].fractions.nFlags & CFractions::NOTARY_V) {
					if (!frOut.SetMark(CFractions::MARK_SET, CFractions::NOTARY_V, nBlockTime)) {
					...
					if (!frOut.SetMark(CFractions::MARK_SET, CFractions::NOTARY_F, nBlockTime)) {
					...
					if (poolFrozen[i].nFairWithdrawFromEscrowIndex1 == i) {
						if (poolFrozen.size() == 2) {
							...
							if (nIndex1 < 0 || nIndex2 < 0 || size_t(nIndex1) >= n_vout ||
							    size_t(nIndex2) >= n_vout) {
								sFailCause =
								    "P-O-N-3: Wrong refering output for fair withdraw from escrow";
								fFailedPegOut = true;
								break;
							}
							...
							if (poolReserves.count(sFrozenAddress) > 0) {
								...
								if (nReserve <= (nValue1 + nValue2) && (nValue1 + nValue2) > 0) {
									int64_t nScaledValue1 =
									    RatioPart(nReserve, nValue1, nValue1 + nValue2);
									int64_t nScaledValue2 =
									    RatioPart(nReserve, nValue2, nValue1 + nValue2);
					...
					for (const string& sAddress : vAddresses) {
						if (poolReserves.count(sAddress) == 0)
							continue;
						auto&   frReserve = poolReserves[sAddress];
						int64_t nReserve  = frReserve.Total();
						if (nReserve == 0)
			...
			if (!poolFrozen.count(i)) {              // not frozen
				if (poolReserves.count(sAddress)) {  // back to reserve
					...
					if (sAddress == sBurnAddress || (fNotary && !fNotaryZ)) {
						vector<string> vAddresses;
						for (auto it = poolReserves.begin(); it != poolReserves.end(); it++) {
							vAddresses.push_back(it->first);
						}
	...
	if (nFee != txFeeFractions.Total()) {
		sFailCause = "P-G-3: Total mismatch on fee fractions " + std::to_string(nFee) + " vs " +
		             std::to_string(txFeeFractions.Total());
		fFailedPegOut = true;
	}
	if (!txFeeFractions.IsPositive()) {
		sFailCause    = "P-G-4: Non-positive fee fractions";
		fFailedPegOut = true;
	}
...
bool CFractions::SetMark(MarkAction action, uint32_t nMark, uint64_t nTime) {
	...
	int nMarks = 0;
	if (nNewFlags & CFractions::NOTARY_F)
		nMarks++;
	if (nNewFlags & CFractions::NOTARY_V)
		nMarks++;
	if (nNewFlags & CFractions::NOTARY_L)
		nMarks++;
	if (nNewFlags & CFractions::NOTARY_C)
		nMarks++;
