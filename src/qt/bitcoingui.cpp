...
BitcoinGUI::BitcoinGUI(QWidget* parent)
	...
	connect(centralStackedWidget, SIGNAL(currentChanged(int)), this, SLOT(onTabChanged(int)));
	...
	header->setSizePolicy(QSizePolicy::Fixed, QSizePolicy::MinimumExpanding);
	header->setStyleSheet(
	    "QWidget { background-color: rgb(75,78,162); background-repeat: no-repeat; "
	    "background-image: url(:/images/header); background-position: top center; }");
	headerLayout->addWidget(header);

	QWidget* space1 = new QWidget();
	space1->setSizePolicy(QSizePolicy::MinimumExpanding, QSizePolicy::Minimum);
	...
	QWidget* w_liquidMeter = new QWidget;
	liquidMeter            = new ReserveMeter(w_liquidMeter);
	liquidMeter->setFixedSize(200, 200);
	w_liquidMeter->setFixedWidth(200);
	topHeaderLayout->addWidget(w_liquidMeter, 0, 7, 3, 1);

	QWidget*     centralWidget = new QWidget();
	QVBoxLayout* centralLayout = new QVBoxLayout(centralWidget);
	centralLayout->addLayout(headerLayout, 1);
	centralLayout->addLayout(centralHLayout, 100);
	...
	connect(labelStakingIcon, SIGNAL(clicked()), this, SLOT(gotoStakingPage()));
	connect(labelConnectionsIcon, SIGNAL(clicked()), this, SLOT(gotoInfoPageNet()));
	connect(labelBlocksIcon, SIGNAL(clicked()), this, SLOT(gotoInfoPageBlocks()));
	...
	statusBar()->setStyleSheet(
	    "QWidget { background-color: rgb(75,78,162); color: rgb(193,193,193); }");
	...
	connect(addressBookPage, SIGNAL(verifyMessage(QString)), this,
	        SLOT(gotoVerifyMessageTab(QString)));
	// Clicking on "Sign Message" in the receive coins page sends you to the sign message tab
	connect(receiveCoinsPage, SIGNAL(signMessage(QString)), this,
	        SLOT(gotoSignMessageTab(QString)));

	gotoOverviewPage();

	// Request rates
	netAccessManager = new QNetworkAccessManager(this);
	connect(netAccessManager, &QNetworkAccessManager::finished, this,
	        &BitcoinGUI::netDataReplyFinished);
...
#ifndef Q_OS_MAC
	QFont f_liquidMeter = liquidMeter->font();
	f_liquidMeter.setPixelSize(16);
	liquidMeter->setFont(f_liquidMeter);
#endif

#ifdef Q_OS_MAC
	liquidMeter->setFixedSize(180, 180);
	w_liquidMeter->setFixedWidth(180);
	QFont f_liquidMeter = liquidMeter->font();
	f_liquidMeter.setPixelSize(9);
	liquidMeter->setFont(f_liquidMeter);
#endif
}
...
void BitcoinGUI::setClientModel(ClientModel* clientModel) {
	...
	if (clientModel) {
		...
		connect(clientModel, SIGNAL(numConnectionsChanged(int)), this,
		        SLOT(setNumConnections(int)));

		setNumBlocks(clientModel->getNumBlocks());
		connect(clientModel, SIGNAL(numBlocksChanged(int)), this, SLOT(setNumBlocks(int)));
		connect(clientModel, SIGNAL(numBlocksChanged(int)), infoPage->blockchainModel(),
		        SLOT(setNumBlocks(int)));
		...
		connect(clientModel, SIGNAL(message(QString, QString, bool, uint32_t)), this,
		        SLOT(message(QString, QString, bool, uint32_t)));
...
void BitcoinGUI::setWalletModel(WalletModel* walletModel) {
	...
	if (walletModel) {
		...
		connect(walletModel, SIGNAL(message(QString, QString, bool, uint32_t)), this,
		        SLOT(message(QString, QString, bool, uint32_t)));
		...
		connect(walletModel, SIGNAL(encryptionStatusChanged(int)), this,
		        SLOT(setEncryptionStatus(int)));

		// Balloon pop-up for new transaction
		connect(walletModel->getTransactionTableModel(),
		        SIGNAL(rowsInserted(QModelIndex, int, int)), this,
		        SLOT(incomingTransaction(QModelIndex, int, int)));
...
void BitcoinGUI::setNumBlocks(int count) {
	...
	if (secs < 90 * 60) {
		...
		labelBlocksIcon->setPixmap(
		    QIcon(":/icons/unsynced").pixmap(STATUSBAR_ICONSIZE, STATUSBAR_ICONSIZE));
		if (count != prevBlocks)
...
void BitcoinGUI::updatePegInfo1Label() {
	...
	int peg_interval                                         = Params().PegInterval();
	int interval_num                                         = last_block_num / peg_interval;
	pegNowTextLabel->setText(tr("Peg index now - %1: ").arg((interval_num + 1) * peg_interval - 1));
...
QString BitcoinGUI::timeBehindText(int secs) {
	...
	if (secs < 2 * MINUTE_IN_SECONDS) {
		...
		time_behind_text = tr("%1 and %2")
		                       .arg(tr("%n year(s)", "", years))
		                       .arg(tr("%n week(s)", "", remainder / WEEK_IN_SECONDS));
	}
	return time_behind_text;
}

void BitcoinGUI::message(const QString& title, const QString& message, bool modal, uint32_t style) {
	...
	if (modal) {
		...
		if (!clientModel->getOptionsModel())
			return;
		OptionsModel* om = clientModel->getOptionsModel();
		if (om->getNotificationsOn()) {
...
void BitcoinGUI::askFee(qint64 nFeeRequired, bool* payFee) {
	...
	QString strMessage =
	    tr("This transaction is over the size limit. You can still send it for a fee of %1, "
	       "which goes to the nodes that process your transaction and helps to support the "
	       "network. "
	       "Do you want to pay the fee?")
	        .arg(BitcoinUnits::formatWithUnit(clientModel->getOptionsModel()->getDisplayUnit(),
	                                          nFeeRequired));
...
void BitcoinGUI::incomingTransaction(const QModelIndex& parent, int start, int end) {
	...
	if (!clientModel->inInitialBlockDownload()) {
		...
		QString address =
		    ttm->index(start, TransactionTableModel::ToAddress, parent).data().toString();
...
void BitcoinGUI::gotoHistoryPage() {
	...
	if (!tabTransactions->isChecked())
		tabTransactions->setChecked(true);
...
void BitcoinGUI::dropEvent(QDropEvent* event) {
	if (event->mimeData()->hasUrls()) {
		...
		if (nValidUrisFound) {
			...
			if (om->getNotificationsOn()) {
				notificator->notify(
				    Notificator::Warning, tr("URI handling"),
				    tr("URI can not be parsed! This can be caused by an invalid BitBay "
...
void BitcoinGUI::netDataReplyFinished(QNetworkReply* reply) {
	...
	if (reply->error()) {
		if (reply->url().path().endsWith("rates1k.json")) {
			dynamicPegPage->setStatusMessage("Algorithm Chart error, " + reply->errorString());
			...
			dynamicPegPage->setStatusMessage("Algorithm URL error, " + reply->errorString());
			dynamicPegPage->setAlgorithmVote("disabled", 0.2);
			if (walletModel) {
				walletModel->setTrackerVote(PEG_VOTE_NONE, 0.2);
			}
	...
	if (reply->url().path().endsWith("rates1k.json")) {
		...
		for (int i = 0; i < records.size(); i++) {
			...
			if (!record_bay.isObject())
				continue;
			if (!record_btc.isObject())
				continue;
			...
			if (!record_bay_price.isDouble())
				continue;
			if (!record_btc_price.isDouble())
				continue;
...

		{
			...
			if ((pindexBest->nTime > nTimeMax) && ((pindexBest->nTime - nTimeMax) > 12 * 3600)) {
				dynamicPegPage->setStatusMessage(
				    "Algorithm Chart is not live, last record of " +
		...
		if (have_rate) {
			...
			for (size_t i = 0; i < btc_rates.size(); ++i)
				v_btc_rates.push_back(btc_rates[i]);
			for (size_t i = 0; i < bay_rates.size(); ++i)
				v_bay_rates.push_back(bay_rates[i]);
		...
		if (have_rate) {
...

			{
				...
				if ((pindexBest->nTime > nTime) && ((pindexBest->nTime - nTime) > 12 * 3600)) {
					dynamicPegPage->setStatusMessage(
					    "Algorithm URL is not live, last record of " +
		...
		if (!record_ver_maj.isDouble())
			return;
		if (!record_ver_min.isDouble())
			return;
		if (!record_ver_rev.isDouble())
			return;
		...
		if (ver_maj < CLIENT_VERSION_MAJOR)
			return;
		if (ver_maj == CLIENT_VERSION_MAJOR && ver_min < CLIENT_VERSION_MINOR)
			return;
		if (ver_maj == CLIENT_VERSION_MAJOR && ver_min == CLIENT_VERSION_MINOR &&
		    ver_rev <= CLIENT_VERSION_REVISION)
			return;
		if (!walletModel) {
			return;
		}
		QMessageBox::warning(this, tr("New Version Warning"),
