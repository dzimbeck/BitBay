...
BlockchainPage::BlockchainPage(QWidget* parent) : QDialog(parent), ui(new Ui::BlockchainPage) {
	...
	connect(ui->lineJumpToBlock, SIGNAL(returnPressed()), this, SLOT(jumpToBlock()));
	connect(ui->lineFindBlock, SIGNAL(returnPressed()), this, SLOT(openBlockFromInput()));
	connect(ui->lineBalanceAddress, SIGNAL(returnPressed()), this, SLOT(openBalanceFromInput()));
	connect(ui->lineUtxoAddress, SIGNAL(returnPressed()), this, SLOT(openUnspentFromInput()));
	connect(ui->lineTx, SIGNAL(returnPressed()), this, SLOT(openTxFromInput()));
	...
	connect(ui->balanceCurrent, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
	        SLOT(openUnspentFromBalance(QTreeWidgetItem*, int)));
	connect(ui->balanceValues, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
	        SLOT(openTxFromBalance(QTreeWidgetItem*, int)));
	connect(ui->utxoValues, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
	        SLOT(openTxFromUnspent(QTreeWidgetItem*, int)));
	...
	connect(ui->balanceCurrent, SIGNAL(customContextMenuRequested(const QPoint&)), this,
	        SLOT(openBalanceMenu1(const QPoint&)));
	connect(ui->balanceValues, SIGNAL(customContextMenuRequested(const QPoint&)), this,
	        SLOT(openBalanceMenu2(const QPoint&)));
	connect(ui->utxoValues, SIGNAL(customContextMenuRequested(const QPoint&)), this,
	        SLOT(openUnspentMenu(const QPoint&)));
...
void BlockchainPage::openChainMenu(const QPoint& pos) {
	...
	connect(a, &QAction::triggered, [&] {
		QApplication::clipboard()->setText(mi.data(BlockchainModel::HashStringRole).toString());
	});
	a = m.addAction(tr("Copy Block Height"));
	connect(a, &QAction::triggered, [&] {
		QApplication::clipboard()->setText(mi.data(BlockchainModel::HeightRole).toString());
	});
...
void BlockchainPage::openBlock(uint256 hash) {
	...
	ui->blockValues->addTopLevelItem(
	    new QTreeWidgetItem(QStringList({"Version", QString::number(block.nVersion)})));
	ui->blockValues->addTopLevelItem(new QTreeWidgetItem(QStringList(
...
void BlockchainPage::setClientModel(ClientModel* model) {
	if (model) {
		connect(model, SIGNAL(connectionsChanged(const CNodeShortStats&)), this,
		        SLOT(updateConnections(const CNodeShortStats&)));
...
void BlockchainPage::updateMempool() {
	...
	for (set<int>::reverse_iterator rit = removeIndexes.rbegin(); rit != removeIndexes.rend();
	     rit++) {
	...
	for (uint256 newTxhash : newTxhashes) {
		if (oldTxhashes.count(newTxhash)) {
			continue;
		}
...

	{
		...
		for (uint256 newTxhash : insertTxhashes) {
			...
			for (size_t i = 0; i < nVin; i++) {
				...
				if (!mempool.mapPrevOuts.count(newTxhash))
					continue;
				if (!mempool.mapPrevOuts.at(newTxhash).count(fkey))
					continue;
...
void BlockchainPage::openBalanceFromTx(QString addr) {
	...
	for (const auto& record : records) {
		if (isLatestRecord) {
			...
			int     nValueMaxLen = qMax(
                displayValue(record.nBalance).length(),
                qMax(displayValue(nLiquid).length(),
			             qMax(displayValue(nReserve).length(), displayValue(record.nFrozen).length())));

			ui->balanceCurrent->addTopLevelItem(new QTreeWidgetItem(
			    QStringList({"Total", displayValueR(record.nBalance, nValueMaxLen)})));
			ui->balanceCurrent->addTopLevelItem(
			    new QTreeWidgetItem(QStringList({"Liquid", displayValueR(nLiquid, nValueMaxLen)})));
			ui->balanceCurrent->addTopLevelItem(new QTreeWidgetItem(
			    QStringList({"Reserve", displayValueR(nReserve, nValueMaxLen)})));
			ui->balanceCurrent->addTopLevelItem(new QTreeWidgetItem(
			    QStringList({"Frozen", displayValueR(record.nFrozen, nValueMaxLen)})));
	...
	if (isLatestRecord) {
		int nValueMaxLen = qMax(displayValue(0).length(),
		                        qMax(displayValue(0).length(),
		                             qMax(displayValue(0).length(), displayValue(0).length())));

		ui->balanceCurrent->addTopLevelItem(
		    new QTreeWidgetItem(QStringList({"Total", displayValueR(0, nValueMaxLen)})));
		ui->balanceCurrent->addTopLevelItem(
		    new QTreeWidgetItem(QStringList({"Liquid", displayValueR(0, nValueMaxLen)})));
		ui->balanceCurrent->addTopLevelItem(
		    new QTreeWidgetItem(QStringList({"Reserve", displayValueR(0, nValueMaxLen)})));
		ui->balanceCurrent->addTopLevelItem(
		    new QTreeWidgetItem(QStringList({"Frozen", displayValueR(0, nValueMaxLen)})));
		isLatestRecord = false;
	}
}
...

	{
		...
		if (ok) {
			...
			for (const auto& record : records) {
				...
				item->setText(
				    1,
				    QString("%1-%2:%3").arg(record.nHeight).arg(record.nIndex).arg(txoutid.b2()));
