...
void TxViewDelegate::paint(QPainter*                   painter,
	...
	QRect amountRect(mainRect.left() + xspace, mainRect.top() + ypad, mainRect.width() - xspace,
	                 halfheight);
	// QRect addressRect(mainRect.left() + xspace, mainRect.top()+ypad+halfheight, mainRect.width()
	// - xspace, halfheight); icon.paint(painter, decorationRect);
...
OverviewPage::OverviewPage(QWidget* parent)
	...
	QFont hfont = GUIUtil::header1Font();
...
void OverviewPage::setBalance(qint64                  balance,
	...
	ui->labelTotal->setText(BitcoinUnits::formatWithUnitForLabel(
	    unit, balance + stake + unconfirmedBalance + immatureBalance));
	...
	if (walletModel && walletModel->getOptionsModel()) {
		if (currentBalance != -1)
			setBalance(currentBalance, currentReserve, currentLiquidity, currentFrozen,
			           currentFrozenCoins, walletModel->getStake(), currentUnconfirmedBalance,
...
void OverviewPage::openFrozenCoinsInfo() {
	...
	for (const auto& coin : frozenCoins) {
		...
		cells << BitcoinUnits::formatWithUnit(walletModel->getOptionsModel()->getDisplayUnit(),
		                                      coin.nValue);
