...
SendCoinsDialog::SendCoinsDialog(QWidget* parent)
	...
	connect(clipboardPriorityAction, SIGNAL(triggered()), this,
	...
	ui->comboBoxTxType->addItem(tr("Transfer spendable funds (liquidity)"),
	                            PEG_MAKETX_SEND_LIQUIDITY);
	ui->comboBoxTxType->addItem(tr("Transfer reserve and freeze it for 1 month"),
	                            PEG_MAKETX_SEND_RESERVE);
	ui->comboBoxTxType->addItem(
	    tr("Freeze reserve for 1 month (such funds have 20 BAY staking reward)"),
	    PEG_MAKETX_FREEZE_RESERVE);
	ui->comboBoxTxType->addItem(
	    tr("Freeze liquidity for 4 month (such funds have 40 BAY staking reward)"),
	    PEG_MAKETX_FREEZE_LIQUIDITY);
	//    ui->comboBoxTxType->insertSeparator(4);
	//    ui->comboBoxTxType->addItem(tr("Transfer coins to cold wallet"), PEG_MAKETX_SEND_TOCOLD);
	//    ui->comboBoxTxType->addItem(tr("Return coins from cold wallet"),
	//    PEG_MAKETX_SEND_FROMCOLD);
...
void SendCoinsDialog::setModel(WalletModel* model) {
	...
	if (model && model->getOptionsModel()) {
		...
		setBalance(model->getBalance(), model->getReserve(), model->getLiquidity(), nFrozen,
		           frozenCoins, model->getStake(), model->getUnconfirmedBalance(),
		           model->getImmatureBalance());
		connect(model,
		        SIGNAL(balanceChanged(qint64, qint64, qint64, qint64, std::vector<CFrozenCoinInfo>,
		                              qint64, qint64, qint64)),
		        this,
		        SLOT(setBalance(qint64, qint64, qint64, qint64, std::vector<CFrozenCoinInfo>,
		                        qint64, qint64, qint64)));
		...
		case WalletModel::AmountWithFeeExceedsBalance:
			QMessageBox::warning(
			    ...
			    tr("The total exceeds your balance when the %1 transaction fee is included.")
			            .arg(BitcoinUnits::formatWithUnit(
			                model->getOptionsModel()->getDisplayUnit(), sendstatus.fee)) +
			        errdetails,
			    QMessageBox::Ok, QMessageBox::Ok);
			break;
		case WalletModel::DuplicateAddress:
			QMessageBox::warning(this, tr("Send Coins"),
			                     tr("Duplicate address found, can only send to each address once "
			                        "per send operation.") +
			                         errdetails,
			                     QMessageBox::Ok, QMessageBox::Ok);
			break;
		...
		case WalletModel::TransactionCommitFailed:
			QMessageBox::warning(this, tr("Send Coins"),
			                     tr("Error: The transaction was rejected. "
			                        ...
			                        "the copy but not marked as spent here.") +
			                         errdetails,
			                     QMessageBox::Ok, QMessageBox::Ok);
			break;
...
void SendCoinsDialog::setBalance(qint64                  balance,
	...
	if (model && model->getOptionsModel()) {
		...
		// balance));
		ui->labelReserves->setText(BitcoinUnits::formatWithUnitForLabel(
		    model->getOptionsModel()->getDisplayUnit(), reserves));
		ui->labelLiquidity->setText(BitcoinUnits::formatWithUnitForLabel(
		    model->getOptionsModel()->getDisplayUnit(), liquidity));
	}
}
