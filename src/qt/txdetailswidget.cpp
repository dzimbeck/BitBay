...
TxDetailsWidget::TxDetailsWidget(QWidget* parent) : QWidget(parent), ui(new Ui::TxDetails) {
	ui->setupUi(this);
	...
	connect(ui->txValues, SIGNAL(customContextMenuRequested(const QPoint&)), this,
	        SLOT(openTxMenu(const QPoint&)));
	connect(ui->txInputs, SIGNAL(customContextMenuRequested(const QPoint&)), this,
	        SLOT(openInpMenu(const QPoint&)));
	connect(ui->txOutputs, SIGNAL(customContextMenuRequested(const QPoint&)), this,
	        SLOT(openOutMenu(const QPoint&)));

	connect(ui->txInputs, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
	        SLOT(openFractions(QTreeWidgetItem*, int)));
	connect(ui->txOutputs, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
	        SLOT(openFractions(QTreeWidgetItem*, int)));
	connect(ui->txOutputs, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
	        SLOT(openPegVotes(QTreeWidgetItem*, int)));
	connect(ui->txInputs, SIGNAL(itemDoubleClicked(QTreeWidgetItem*, int)), this,
...
void TxDetailsWidget::showNotFound() {
	...
	ui->txValues->addTopLevelItem(
	    new QTreeWidgetItem(QStringList({"Error", "Transaction not found"})));
}
...
void TxDetailsWidget::openTx(uint256 blockhash, uint txidx) {
	...
	openTx(tx, pblockindex, txidx, nCycle, pblockindex->nPegSupplyIndex,
	       pblockindex->GetNextIntervalPegSupplyIndex(),
	       pblockindex->GetNextNextIntervalPegSupplyIndex(), pblockindex->nTime);
}
...
void TxDetailsWidget::openTx(CTransaction& tx,
	...
	if (pblockindex) {
		...
		ui->txValues->addTopLevelItem(new QTreeWidgetItem(QStringList(
		    {"Datetime", QString::fromStdString(DateTimeStrFormat(pblockindex->GetBlockTime()))})));
		...
		ui->txValues->addTopLevelItem(
		    new QTreeWidgetItem(QStringList({"Confirmations", QString::number(nConfirmations)})));
	...
	if (tx.IsCoinStake() && !pblockindex) {
		ui->txValues->addTopLevelItem(
		    new QTreeWidgetItem(QStringList({"Error", "No blockindex provided"})));
		return;
	}
	...
	for (uint32_t i = 0; i < n_vin; i++) {
		...
		if (mapInputs.find(prevout.hash) != mapInputs.end()) {
			...
			if (prevout.n < txPrev.vout.size()) {
				...
				auto addr =
				    scriptToAddress(txPrev.vout[prevout.n].scriptPubKey, is_notary, is_voting);
				if (addr.isEmpty())
					row << "N/A";  // address, 2
				else {
					row << addr;
	...
	if (tx.IsCoinStake()) {
		...
		inputMined->setData(COL_INP_VALUE, Qt::TextAlignmentRole,
		                    int(Qt::AlignVCenter | Qt::AlignRight));
		...
		inputFees->setData(COL_INP_VALUE, Qt::TextAlignmentRole,
		                   int(Qt::AlignVCenter | Qt::AlignRight));
	...
	for (int i = 0; i < ui->txInputs->topLevelItemCount(); i++) {
		...
		int64_t liquidity = fractions.High(nSupply);
		input->setData(COL_INP_FRACTIONS, Qt::TextAlignmentRole,
		               int(Qt::AlignVCenter | Qt::AlignRight));
		input->setText(COL_INP_FRACTIONS,
		               displayValue(reserve) + " / " + displayValueR(liquidity, nAlignInpHigh));
	}
	...
	for (uint32_t i = 0; i < n_vout; i++) {
			...
			if (txSpend.ReadFromDisk(txpos)) {
				...
				for (const CTxIn& txin : txSpend.vin) {
					if (txin.prevout.hash == hash && txin.prevout.n == i) {
						...
						QString next_txid =
						    next_txid_height.isEmpty() ? next_txid_hash : next_txid_height;
		...
		output->setData(COL_OUT_VALUE, Qt::TextAlignmentRole,
		                int(Qt::AlignVCenter | Qt::AlignRight));
	...
	int nValueMaxLen =
	    qMax(displayValue(nValueIn).length(),
	         qMax(displayValue(nValueOut).length(),
	              qMax(displayValue(nReserveIn).length(), displayValue(nLiquidityIn).length())));
	...
	auto twiPegChecks = new QTreeWidgetItem(QStringList(
	    {"Peg Checks", peg_ok   ? "OK"
	                   : pruned ? "SKIP (pruned)"
	                            : "FAIL (" + QString::fromStdString(sPegFailCause) + ")"}));
	ui->txValues->addTopLevelItem(twiPegChecks);
	if (!pruned) {
		ui->txValues->addTopLevelItem(new QTreeWidgetItem(
		    QStringList({"Peg Checks Time", QString::number(msecsPegChecks) + " msecs"})));
	}
	ui->txValues->addTopLevelItem(new QTreeWidgetItem(QStringList(
	    {"Fetch Inputs Time", QString::number(msecsFetchInputs) + " msecs (can be cached)"})));
	...
	if (!tx.IsCoinBase() && !tx.IsCoinStake() /*&& nValueOut < nValueIn*/) {
		...
		outputFees->setData(COL_OUT_VALUE, Qt::TextAlignmentRole,
		                    int(Qt::AlignVCenter | Qt::AlignRight));
		outputFees->setData(COL_OUT_VALUE, BlockchainModel::ValueForCopy,
		                    qlonglong(nValueIn - nValueOut));
	...
	for (int i = 0; i < ui->txOutputs->topLevelItemCount(); i++) {
		...
		int64_t reserve   = fractions.Low(nSupply);
		int64_t liquidity = fractions.High(nSupply);
		output->setData(COL_OUT_FRACTIONS, Qt::TextAlignmentRole,
		                int(Qt::AlignVCenter | Qt::AlignRight));
		output->setText(COL_OUT_FRACTIONS,
		                displayValue(reserve) + " / " + displayValueR(liquidity, nAlignOutHigh));
	}
}
...
void TxDetailsWidget::openInpMenu(const QPoint& pos) {
	...
	connect(a, &QAction::triggered, [&] {
		...
		QApplication::clipboard()->setText(mi2.data(BlockchainModel::ValueForCopy).toString());
	});
	...
	connect(a, &QAction::triggered, [&] {
		...
		QApplication::clipboard()->setText(mi2.data(Qt::DisplayRole).toString());
	});
...
void TxDetailsWidget::openPegVotes(QTreeWidgetItem* item, int column) {
	...
	if (dest != "peginflate" && dest != "pegdeflate" && dest != "pegnochange")
		return;
	...
	int nAlignOutHigh = 0;
	{
		...
		if (text1.length() > nAlignOutHigh)
			nAlignOutHigh = text1.length();
		if (text2.length() > nAlignOutHigh)
			nAlignOutHigh = text2.length();
		if (text3.length() > nAlignOutHigh)
			nAlignOutHigh = text3.length();
		if (text4.length() > nAlignOutHigh)
			nAlignOutHigh = text4.length();
	}
...
void TxDetailsWidget::openFractions(QTreeWidgetItem* item, int column) {
	...
	connect(ui.fractions, SIGNAL(customContextMenuRequested(const QPoint&)), this,
	        SLOT(openFractionsMenu(const QPoint&)));
...
void TxDetailsWidget::plotFractions(QTreeWidget*      table,
	...
	if (packedLabel)
		packedLabel->setText(tr("Packed: %1 bytes").arg(len_test));
	if (valueLabel)
		valueLabel->setText(tr("Value: %1").arg(displayValue(fractions.Total())));
	if (reserveLabel)
		reserveLabel->setText(tr("Reserve: %1").arg(displayValue(fractions.Low(level))));
	if (liquidityLabel)
		liquidityLabel->setText(tr("Liquidity: %1").arg(displayValue(fractions.High(level))));
	...
	if (!simple) {
		...
		row_item_liquid_calc->setData(0, Qt::TextAlignmentRole,
		                              int(Qt::AlignVCenter | Qt::AlignLeft));
		row_item_liquid_calc->setData(1, Qt::TextAlignmentRole,
		                              int(Qt::AlignVCenter | Qt::AlignRight));
		row_item_liquid_calc->setData(1, BlockchainModel::ValueForCopy,
		                              qlonglong(fractions.High(level)));
		...
		row_item_reserve_calc->setData(0, Qt::TextAlignmentRole,
		                               int(Qt::AlignVCenter | Qt::AlignLeft));
		row_item_reserve_calc->setData(1, Qt::TextAlignmentRole,
		                               int(Qt::AlignVCenter | Qt::AlignRight));
		row_item_reserve_calc->setData(1, BlockchainModel::ValueForCopy,
		                               qlonglong(fractions.Low(level)));
		...
		row_item_liquid_save->setData(0, Qt::TextAlignmentRole,
		                              int(Qt::AlignVCenter | Qt::AlignLeft));
		row_item_liquid_save->setData(1, Qt::TextAlignmentRole,
		                              int(Qt::AlignVCenter | Qt::AlignRight));
		...
		row_item_reserve_save->setData(0, Qt::TextAlignmentRole,
		                               int(Qt::AlignVCenter | Qt::AlignLeft));
		row_item_reserve_save->setData(1, Qt::TextAlignmentRole,
		                               int(Qt::AlignVCenter | Qt::AlignRight));
	...
	row_item_lhli->setData(1, BlockchainModel::ValueForCopy,
	                       qlonglong(fractions.HighPart(level, nullptr).HLI()));
	...
	row_item_rhli->setData(1, BlockchainModel::ValueForCopy,
	                       qlonglong(fractions.LowPart(level, nullptr).HLI()));
	...
	if (fractions.nFlags & CFractions::NOTARY_F || fractions.nFlags & CFractions::NOTARY_V ||
	...
	curvePeg->setSamples(xs_peg, ys_peg, 2);
	curveReserve->setSamples(xs_reserve, ys_reserve, supply * 2);
	curveLiquid->setSamples(xs_liquidity + supply * 2, ys_liquidity + supply * 2,
	                        PEG_SIZE * 2 - supply * 2);
	fplot->replot();
}
...
void TxDetailsWidget::copyPegData(QTreeWidget* table) {
	...
	for (int i = 0; i < n; i++) {
		...
		if (name == tr("Cycle"))
			pegdata.peglevel.nCycle = value.toInt();
		if (name == tr("Prev"))
			pegdata.peglevel.nCyclePrev = value.toInt();
		if (name == tr("Peg"))
			pegdata.peglevel.nSupply = value.toInt();
		if (name == tr("PegN"))
			pegdata.peglevel.nSupplyNext = value.toInt();
		if (name == tr("PegNN"))
			pegdata.peglevel.nSupplyNextNext = value.toInt();
		if (name == tr("Buffer"))
			pegdata.peglevel.nBuffer = value.toInt();
		if (name == tr("Shift"))
			pegdata.peglevel.nShift = value.toInt();
		if (name == tr("Part"))
			pegdata.peglevel.nShiftLastPart = value.toInt();
		if (name == tr("PTot"))
			pegdata.peglevel.nShiftLastTotal = value.toInt();
		if (name == tr("Lsave"))
			pegdata.nLiquid = value.toLongLong();
		if (name == tr("Rsave"))
			pegdata.nReserve = value.toLongLong();
		...
		try {
			...
			if (pegdata.IsValid()) {
				...
				connect(a, &QAction::triggered, [&] {
					...
					plotFractions(table, pegdata.fractions, pegdata.peglevel, pegdata.nLiquid,
					              pegdata.nReserve, pegdata.nId);
				});
				a = m.addAction(tr("Copy pegdata"));
				connect(a, &QAction::triggered, [&] { copyPegData(table); });
			}
		} catch (std::exception&) {
			;
		}
