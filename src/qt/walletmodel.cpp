...
void WalletModel::checkBalanceChanged() {
	...
	if (cachedBalance != newBalance || cachedReserve != newReserve ||
	    cachedLiquidity != newLiquidity || cachedFrozen != newFrozen ||
	    cachedFrozenCoins != newFrozenCoins || cachedStake != newStake ||
		...
		cachedUnconfirmedBalance = newUnconfirmedBalance;
		cachedImmatureBalance    = newImmatureBalance;
		emit balanceChanged(newBalance, newReserve, newLiquidity, newFrozen, newFrozenCoins,
	...
	if (cachedRewardsInfo.size() != PEG_REWARD_LAST) {
		...
		for (int i = 0; i < PEG_REWARD_LAST; i++) {
			if (vRewardsInfo[i].amount != cachedRewardsInfo[i].amount)
				changed = true;
			if (vRewardsInfo[i].count != cachedRewardsInfo[i].count)
				changed = true;
			if (vRewardsInfo[i].stake != cachedRewardsInfo[i].stake)
				changed = true;
		}
	}

	if (changed) {
		emit rewardsInfoChanged(
		    vRewardsInfo[PEG_REWARD_5].amount, vRewardsInfo[PEG_REWARD_10].amount,
		    vRewardsInfo[PEG_REWARD_20].amount, vRewardsInfo[PEG_REWARD_40].amount,

		    vRewardsInfo[PEG_REWARD_5].count, vRewardsInfo[PEG_REWARD_10].count,
		    vRewardsInfo[PEG_REWARD_20].count, vRewardsInfo[PEG_REWARD_40].count,

		    vRewardsInfo[PEG_REWARD_5].stake, vRewardsInfo[PEG_REWARD_10].stake,
		    vRewardsInfo[PEG_REWARD_20].stake, vRewardsInfo[PEG_REWARD_40].stake);
	}
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient>& recipients,
	...
	if ((total + nTransactionFee) >
	    nBalanceAll)  // fee can be paid from both reserves and liquidity
	{
...
static void NotifyAddressBookChanged(WalletModel*          walletmodel,
	...
	qDebug() << "NotifyAddressBookChanged : " + strAddress + " " + strLabel +
	                " isMine=" + QString::number(isMine) + " status=" + QString::number(status);
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const {
	std::vector<COutput> vCoins;
	wallet->AvailableCoins(vCoins, /*fOnlyConfirmed*/ true, /*fUseFrozenUnlocked*/ true,
	                       /*coinControl*/ NULL);
