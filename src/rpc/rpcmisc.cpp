...
Value getfractions(const Array& params, bool fHelp) {
	...
	for (int i = 0; i < PEG_SIZE; i++) {
		total += fractions.f[i];
		if (i < supply)
			reserve += fractions.f[i];
		if (i >= supply)
			liquidity += fractions.f[i];
...
Value getliquidityrate(const Array& params, bool fHelp) {
	...
	for (int i = supply; i <= highkey; i++) {
		...
		while (true) {
			if (multiplier > 20)
				break;
			if (average <= 0.1 * multiplier)
				break;
			if (periods.size() != size_t((multiplier - 1) * 2))
				break;
...
Value validaterawtransaction(const Array& params, bool fHelp) {
	if (fHelp || params.size() != 2)
		throw runtime_error(
		    ...
		    "Second argument is peg supply level for the validation of the transaction with peg "
		    "system.\n"
