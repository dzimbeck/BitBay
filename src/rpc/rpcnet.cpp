...
Value addnode(const Array& params, bool fHelp) {
	...
	if (fHelp || params.size() != 2 ||
		...
		throw runtime_error(
		    ...
		    "Attempts add or remove <node> from the addnode list or try a connection to <node> "
		    "once.");
...
// ThreadRPCServer: holds cs_main and acquiring cs_vSend in
// alert.RelayTo()/PushMessage()/BeginMessage()
Value sendalert(const Array& params, bool fHelp) {
	...
	key.SetPrivKey(CPrivKey(vchPrivKey.begin(), vchPrivKey.end()),
	               false);  // if key is not correct openssl may crash
