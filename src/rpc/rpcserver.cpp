...
void RPCTypeCheck(const Array& params, const list<Value_type>& typesExpected, bool fAllowNull) {
	...
	for (const Value_type& t : typesExpected) {
		...
		if (!((v.type() == t) || (fAllowNull && (v.type() == null_type)))) {
			string err = strprintf("Expected type %s, got %s", Value_type_name[t],
...
static void RPCListen(boost::shared_ptr<basic_socket_acceptor<Protocol> > acceptor,
	...
	AcceptedConnectionImpl<Protocol>* conn =
	    new AcceptedConnectionImpl<Protocol>(GetIOServiceFromPtr(acceptor), context, fUseSSL);
...
static void RPCAcceptHandler(boost::shared_ptr<basic_socket_acceptor<Protocol> > acceptor,
                             ...
                             AcceptedConnection*                                 conn,
                             const boost::system::error_code&                    error) {
	// Immediately start accepting new connections, except when we're cancelled or our socket is
	// closed.
...
void StartRPCThreads() {
	...
	if (((mapArgs["-rpcpassword"] == "") || (mapArgs["-rpcuser"] == mapArgs["-rpcpassword"])) &&
		...
		string strWhatAmI = "To use bitbayd";
	...
	if (fUseSSL) {
		...
		string strCiphers = GetArg("-rpcsslciphers",
	...
	try {
		...
		strerr = strprintf(_("An error occurred while setting up the RPC port %u for listening on "
		                     "IPv6, falling back to IPv4: %s"),
		                   endpoint.port(), e.what());
	}

	try {
		// If dual IPv6/IPv4 failed (or we're opening loopback interfaces only), open IPv4
		// separately
...
void RPCRunLater(const std::string& name, boost::function<void(void)> func, int64_t nSeconds) {
	...
	if (deadlineTimers.count(name) == 0) {
		deadlineTimers.insert(make_pair(
		    name, boost::shared_ptr<deadline_timer>(new deadline_timer(*rpc_io_service))));
	}
