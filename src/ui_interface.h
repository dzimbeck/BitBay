...
public:
	...
	enum MessageBoxFlags {
		 ...
		 */
		BTN_MASK = (BTN_OK | BTN_YES | BTN_NO | BTN_ABORT | BTN_RETRY | BTN_IGNORE | BTN_CLOSE |
		            BTN_CANCEL | BTN_DISCARD | BTN_HELP | BTN_APPLY | BTN_RESET),
	...
	boost::signals2::signal<
	    void(const std::string& message, const std::string& caption, unsigned int style)>
	...
	boost::signals2::signal<void(const std::vector<CNodeShortStat>& newConnections)>
...
/**
 * Translation function: Call Translate signal on UI interface, which returns a boost::optional
 * result. If no translation slot is registered, nothing is returned, and simply return the input.
 */
