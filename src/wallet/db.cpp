...
bool CDBEnv::Open(boost::filesystem::path pathEnv_) {
	...
	int ret = dbenv.open(strPath.c_str(),
	                     ...
	                     S_IRUSR | S_IWUSR);
	if (ret != 0)
		return error("CDB() : error %s (%d) opening database environment", DbEnv::strerror(ret),
		             ret);
...
CDBEnv::VerifyResult CDBEnv::Verify(std::string strFile,
                                    bool (*recoverFunc)(CDBEnv& dbenv, std::string strFile)) {
	LOCK(cs_db);
...

	{
		...
		if (pdb == NULL) {
			...
			if (ret != 0) {
				...
				strFile = "";
				throw runtime_error(
				    strprintf("CDB : Error %d, can't open database %s", ret, strFile));
			}
