...
Value getstakinginfo(const Array& params, bool fHelp) {
	...
	uint64_t nExpectedTime =
	    staking ? (GetTargetSpacing(nBestHeight) * nNetworkWeight / nWeight) : 0;
...
Value checkkernel(const Array& params, bool fHelp) {
	...
	if (vNodes.empty())
		throw JSONRPCError(-9, "BitBay is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(-10, "BitBay is downloading blocks...");
...
Value getworkex(const Array& params, bool fHelp) {
	...
	if (params.size() == 0) {
		...
		mapNewBlock[pblock->hashMerkleRoot] =
		    make_pair(pblock.get(), pblock->vtx[0].vin[0].scriptSig);
...
Value getwork(const Array& params, bool fHelp) {
	...
	if (vNodes.empty())
		throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "BitBay is not connected!");

	if (IsInitialBlockDownload())
		throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "BitBay is downloading blocks...");
	...
	if (params.size() == 0) {
		...
		if (pindexPrev != pindexBest ||
			...
			// Clear pindexPrev so future getworks make a new block, despite any failures from here
			// on
			pindexPrev = NULL;
...
Value getblocktemplate(const Array& params, bool fHelp) {
	if (fHelp || params.size() > 1)
		throw runtime_error(
		    ...
		    "  \"transactions\" : contents of non-coinbase transactions that should be included in "
		    "the next block\n"
		    "  \"coinbaseaux\" : data that should be included in coinbase\n"
		    "  \"coinbasevalue\" : maximum allowable input to coinbase transaction, including the "
