...
static void accountingIsActiveCheck() {
	...
	if (GetBoolArg("-staking", true))
		throw runtime_error(
		    "If you want to use accounting API, staking must be disabled, add to your config file "
		    "staking=0\n");
#endif
}
...
void WalletTxToJSON(const CWalletTx& wtx, Object& entry) {
	...
	if (confirms > 0) {
		...
		entry.push_back(Pair("blocktime", (int64_t)(mapBlockIndex.ref(wtx.hashBlock)->nTime)));
	}
...
Value getnewaddress(const Array& params, bool fHelp) {
	if (fHelp || params.size() > 1)
		throw runtime_error(
		    ...
		    "Returns a new BitBay address for receiving payments.  "
...
Value getaccountaddress(const Array& params, bool fHelp) {
	if (fHelp || params.size() != 1)
		throw runtime_error(
		    ...
		    "Returns the current BitBay address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp) {
	if (fHelp || params.size() < 1 || params.size() > 2)
		throw runtime_error(
		    "setaccount <bitbayaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp) {
	if (fHelp || params.size() < 2 || params.size() > 4)
		throw runtime_error(
		    "sendtoaddress <bitbayaddress> <amount> [comment] [comment-to]\n"
...
Value signmessage(const Array& params, bool fHelp) {
	if (fHelp || params.size() != 2)
		throw runtime_error(
		    "signmessage <bitbayaddress> <message>\n"
...
Value getreceivedbyaddress(const Array& params, bool fHelp) {
	if (fHelp || params.size() < 1 || params.size() > 2)
		throw runtime_error(
		    "getreceivedbyaddress <bitbayaddress> [minconf=1]\n"
		    "Returns the total amount received by <bitbayaddress> in transactions with at least "
...
Value getreceivedbyaccount(const Array& params, bool fHelp) {
	if (fHelp || params.size() < 1 || params.size() > 2)
		throw runtime_error(
		    ...
		    "Returns the total amount received by addresses with <account> in transactions with at "
...
Value sendfrom(const Array& params, bool fHelp) {
	if (fHelp || params.size() < 3 || params.size() > 6)
		throw runtime_error(
		    "sendfrom <fromaccount> <tobitbayaddress> <amount> [minconf=1] [comment] [comment-to]\n"
		    "<amount> is a real and is rounded to the nearest 0.000001" +
		    HelpRequiringPassphrase());
	...
	for (const Pair& s : sendTo) {
		...
		if (!address.IsValid())
			throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY,
			                   string("Invalid BitBay address: ") + s.name_);
...
Value addmultisigaddress(const Array& params, bool fHelp) {
	if (fHelp || params.size() < 2 || params.size() > 3) {
		string msg =
		    ...
		    "each key is a BitBay address or hex-encoded public key\n"
...
Value listreceivedbyaddress(const Array& params, bool fHelp) {
	if (fHelp || params.size() > 3)
		throw runtime_error(
		    ...
		    "[includewatchonly] whether to include watchonly addresses (see 'importaddress').\n"
		    "Returns an array of objects containing:\n"
		    "  \"involvesWatchonly\" : \"true\", (bool) only returned if imported addresses were "
...
Value listsinceblock(const Array& params, bool fHelp) {
	if (fHelp)
		throw runtime_error(
		    ...
		    "Get all transactions in blocks since block [blockhash], or all transactions if "
		    "omitted");
...
Value walletpassphrase(const Array& params, bool fHelp) {
	...
	if (fHelp)
		return true;
	if (!fServer)
		throw JSONRPCError(RPC_SERVER_NOT_STARTED,
		                   "Error: RPC server was not started, use server=1 to change this.");
...
Value encryptwallet(const Array& params, bool fHelp) {
	...
	StartShutdown();
	return "wallet encrypted; BitBay server stopping, restart to run with encrypted wallet. The "
...
Value reservebalance(const Array& params, bool fHelp) {
	...
	Object result;
	result.push_back(Pair("nostake", (nNoStakeBalance > 0)));
	result.push_back(Pair("amount", ValueFromAmount(nNoStakeBalance)));
	return result;
}
