...
Value getreceivedbyaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            ...
            "Returns the total amount received by <bitbayaddress> in transactions with at least [minconf] confirmations.");
...
Value sendmany(const Array& params, bool fHelp)
{
    ...
    for(const Pair& s : sendTo)
    {
        ...
        if (!address.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, string("Invalid BitBay address: ")+s.name_);
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; BitBay server stopping, restart to run with encrypted wallet. The keypool has been flushed, you need to make a new backup.";
}
