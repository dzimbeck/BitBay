...
bool ReadKeyValue(CWallet*          pwallet,
	...
	try {
		...
		if (strType == "name") {
			...
			// ... which was slow for wallets with lots of keys, because the public key is
			// re-derived from the private key using EC operations as a checksum. Newer wallets
			// store keys as "key"[pubkey] => [privkey][hash(pubkey,privkey)], which is much faster
...
bool CWalletDB::Recover(CDBEnv& dbenv, std::string filename, bool fOnlyKeys) {
	...
	int result =
	    dbenv.dbenv.dbrename(NULL, filename.c_str(), NULL, newFilename.c_str(), DB_AUTO_COMMIT);
	if (result == 0)
	...
	for (CDBEnv::KeyValPair& row : salvagedData) {
		if (fOnlyKeys) {
			...
			string      strType, strErr;
			bool        fReadOK =
			    ReadKeyValue(&dummyWallet, ssKey, ssValue, wss, strType, strErr, txdb, pegdb);
			if (!IsKeyType(strType))
				continue;
			if (!fReadOK) {
